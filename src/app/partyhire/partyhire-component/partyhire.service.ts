...
export class PartyHireService {
  ...
  private readonly fallbackStock: PartyHireStockItem[] =
    structuredClone(DEFAULT_STOCK);
  private readonly fallbackOrders: PartyHireOrder[] = [];
  private lastFetched: {
    inventory: PartyHireStockItem[];
  ...
  async listStock(): Promise<PartyHireStockItem[]> {
    ...
    return this.fallbackStock.map((item) => ({
      ...item,
      available: item.total - item.allocated,
    }));
  }
  ...
  async createOrder(payload: NewPartyHireOrder): Promise<PartyHireOrder> {
    const sessionOrder = await this.request<PartyHireMutationResponse>(
      'create_order',
      {
        ...payload,
  ...
  async updateStatus(
    ...
    const response = await this.request<PartyHireMutationResponse>(
      'update_status',
      {
        id: orderId,
        status,
  ...
  async regenerateCalendar(orderId: string): Promise<void> {
    await this.request<PartyHireMutationResponse>('regenerate_calendar', {
      id: orderId,
    });
    this.lastFetched = null;
  }
...
${payload.notes ? payload.notes.replace(/\n/g, '<br>') : '—'}
  ...
  private findLocalOrder(orderId: string): PartyHireOrder {
    const order = this.fallbackOrders.find(
      (existing) => existing.id === orderId,
    );
